  ...
  if (inputValue.length > 0 || hasFile) {
    ...
    $.ajax({
      ...
      error: () => {
        ...
        // error log
        console.error(
          "Falaha ao enviar a Mensagem! pPor favor verifique a tua conexão com a internet"
        );
      },
    });
  }
...
function star(user_id) {
  if (getMessengerId() != auth_id) {
    $.ajax({
      ...
      error: () => {
        console.error("Erro no servidor, verifique sua conexão com a internet.");
      },
    });
  }
}
...
function deleteConversation(id) {
  $.ajax({
    ...
    success: (data) => {
      ...
      if (!data.deleted)
        return alert("Ocorreu um erro, as mensagens não podem ser excluídas!");
...
function deleteMessage(id) {
  $.ajax({
    ...
    success: (data) => {
      ...
      if (!data.deleted)
        console.error("Ocorreu um erro, as mensagens não podem ser excluídas!");
