    ...
    <section class="section">
        ...
        <div class="row">
                                    ...
                                    @foreach ($salas as $item)
                                            ...
                                            <td>
                                                <div>
                                                    <form id="deleteForm-{{ $item->id }}"
                                                        action="{{ route('salas.destroy', $item) }}" method="POST">
                                                        @method('DELETE')
                                                        @csrf
                                                        <a href="{{ route('salas.edit', $item->id) }}" title="Actualizar"
                                                        ...
                                                        <a href="{{ route('salas.show', $item->id) }}" title="Detalhes"
                                        ...
                                        <script>
                                            document.getElementById("swal__{{ $item->id }}").addEventListener("click", function(event) {
                                                ...
                                                swal({
                                                    title: 'Tem certeza?',
                                                    text: 'Tem certeza que deseja excluir o curso" {{ $item->descricao }}?" Esta ação não pode ser desfeita.',
                                                    icon: 'warning',
                                                    buttons: {
                                                        cancel: {
                                                            text: 'Não',
            ...
            <div class="col-12 d-flex align-items-center">
                <a class="btn btn-primary mr-2" href="{{ route('salas.create') }}" role="button">+ Novo</a>
                <a class="btn btn-danger" href="{{ route('salas.pdf') }}" role="button">
