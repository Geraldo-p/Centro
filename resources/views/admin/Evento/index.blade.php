    ...
    <section class="section">
        ...
        <div class="row">
                                    ...
                                    @foreach ($eventos as $item)
                                            ...
                                            <td>
                                                <div>
                                                    <form id="deleteForm-{{ $item->id }}"
                                                        action="{{ route('eventos.destroy', $item) }}" method="POST">
                                                        @method('DELETE')
                                                        @csrf
                                                        <a href="{{ route('eventos.edit', $item) }}" title="Actualizar"
                                                        ...
                                                        <a href="{{ route('eventos.show', $item->id) }}" title="Detalhes"
                                        ...
                                        <script>
                                            document.getElementById("swal__{{ $item->id }}").addEventListener("click", function(event) {
                                                ...
                                                swal({
                                                    title: 'Tem certeza?',
                                                    text: 'Tem certeza que deseja excluir o Evento" {{ $item->nome }}?" Esta ação não pode ser desfeita.',
                                                    icon: 'warning',
                                                    buttons: {
                                                        cancel: {
                                                            text: 'Não',
            ...
            <div class="col-12 d-flex align-items-center">
                <a class="btn btn-primary mr-2" href="{{ route('eventos.create') }}" role="button">+ Novo</a>
                <a class="btn btn-danger" href="{{ route('eventos.pdf') }}" role="button">
