...
@section('content')
    ...
    <div class="card text-left">
        <div class="card-body">
            <form action="{{ route('matriculas.store') }}" method="POST" id="InserirForm" enctype="multipart/form-data">
                @csrf
                <h4>Matricular Estudantes</h4>
                <hr>
                <div class="row profile-row">
                    <div class="col-md-6 relative">
                        <div class="profile-feed">
                            <center>
                                <h5>LISTA DOS ESTUDANTES A SEREM MATRICULADOS</h5>
                            </center>
                            ...
                            {{-- <div class="col-sm-12 col-md-3">
                                <div class="form-group mb-3">
                                    ...
                                    <select class="form-control @error('sala_id') is-invalid @enderror" name="sala_id"
                                        id="sala_id" disabled>
                                        @foreach ($turmas as $item)
                                            <option >{{ $item->salas->capacidade - $item->matriculas->count() }}</option>
                                        @endforeach
                                    </select>
                                    @error('sala_id')
                                        <div class="invalid-feedback">{{ $message }}</div>
